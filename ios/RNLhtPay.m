...
/**
   ...
   * @param id
   */
RCT_EXPORT_METHOD(setWxId:(NSString *)wxid universalLink:(NSString *)link){
      wxOpenId = wxid;
      urlLink = link;
//      [WXApi registerApp:wxid];
        [WXApi registerApp:wxid universalLink:link];
...
RCT_EXPORT_METHOD(WXIstall:(NSString *) ins callBack:(RCTResponseSenderBlock)callback){
    wxCallBack = callback;
    Boolean install = [WXApi isWXAppInstalled];
    NSDictionary *events = @{@"install": install ? @"1" : @"0"};
    callback(@[events]);
